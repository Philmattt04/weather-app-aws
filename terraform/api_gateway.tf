...
locals {
  lambda_uri = {
    ...
    weather        = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/${aws_lambda_function.weather.arn}/invocations"
    weather_range  = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/${aws_lambda_function.weather_range.arn}/invocations"
    youtube        = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/${aws_lambda_function.youtube.arn}/invocations"
  }
  cors_headers = {
...
resource "aws_api_gateway_resource" "weather_range" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  parent_id   = aws_api_gateway_rest_api.main.root_resource_id
  path_part   = "weather-range"
}
resource "aws_api_gateway_method" "weather_range_get" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
  resource_id   = aws_api_gateway_resource.weather_range.id
  http_method   = "GET"
  authorization = "NONE"
}
resource "aws_api_gateway_integration" "weather_range_get" {
  rest_api_id             = aws_api_gateway_rest_api.main.id
  resource_id             = aws_api_gateway_resource.weather_range.id
  http_method             = aws_api_gateway_method.weather_range_get.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = local.lambda_uri.weather_range
}
resource "aws_api_gateway_method" "weather_range_options" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
  resource_id   = aws_api_gateway_resource.weather_range.id
  http_method   = "OPTIONS"
  authorization = "NONE"
}
resource "aws_api_gateway_integration" "weather_range_options" {
  rest_api_id       = aws_api_gateway_rest_api.main.id
  resource_id       = aws_api_gateway_resource.weather_range.id
  http_method       = aws_api_gateway_method.weather_range_options.http_method
  type              = "MOCK"
  request_templates = { "application/json" = "{\"statusCode\": 200}" }
}
resource "aws_api_gateway_method_response" "weather_range_options_200" {
  rest_api_id         = aws_api_gateway_rest_api.main.id
  resource_id         = aws_api_gateway_resource.weather_range.id
  http_method         = aws_api_gateway_method.weather_range_options.http_method
  status_code         = "200"
...
resource "aws_api_gateway_integration_response" "weather_range_options" {
  rest_api_id         = aws_api_gateway_rest_api.main.id
  resource_id         = aws_api_gateway_resource.weather_range.id
  http_method         = aws_api_gateway_method.weather_range_options.http_method
  status_code         = "200"
  response_parameters = local.cors_response_params
  depends_on          = [aws_api_gateway_integration.weather_range_options]
}
...
resource "aws_api_gateway_method" "youtube_get" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
  resource_id   = aws_api_gateway_resource.youtube.id
  http_method   = "GET"
  authorization = "NONE"
}
resource "aws_api_gateway_integration" "youtube_get" {
  rest_api_id             = aws_api_gateway_rest_api.main.id
  resource_id             = aws_api_gateway_resource.youtube.id
  http_method             = aws_api_gateway_method.youtube_get.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = local.lambda_uri.youtube
}
resource "aws_api_gateway_method" "youtube_options" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
  resource_id   = aws_api_gateway_resource.youtube.id
  http_method   = "OPTIONS"
  authorization = "NONE"
}
resource "aws_api_gateway_integration" "youtube_options" {
  rest_api_id       = aws_api_gateway_rest_api.main.id
  resource_id       = aws_api_gateway_resource.youtube.id
  http_method       = aws_api_gateway_method.youtube_options.http_method
  type              = "MOCK"
  request_templates = { "application/json" = "{\"statusCode\": 200}" }
}
resource "aws_api_gateway_method_response" "youtube_options_200" {
  rest_api_id         = aws_api_gateway_rest_api.main.id
  resource_id         = aws_api_gateway_resource.youtube.id
  http_method         = aws_api_gateway_method.youtube_options.http_method
  status_code         = "200"
...
resource "aws_api_gateway_integration_response" "youtube_options" {
  rest_api_id         = aws_api_gateway_rest_api.main.id
  resource_id         = aws_api_gateway_resource.youtube.id
  http_method         = aws_api_gateway_method.youtube_options.http_method
  status_code         = "200"
  response_parameters = local.cors_response_params
  depends_on          = [aws_api_gateway_integration.youtube_options]
}
...
resource "aws_api_gateway_deployment" "main" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_integration.weather_range_get,
      aws_api_gateway_integration.youtube_get,
    ]))
  }
